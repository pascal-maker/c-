using CTANDAI.CarReservationSystem.NonAsync.Repositories; // For ICarRepository interface
...
namespace CTANDAI.CarReservationSystem.NonAsync.Services
{
    ...
    public class CarService : ICarService
    {
        ...
        // Import cars from external API and save them to the database (synchronous version)
        public List<Car> ImportCarsFromApi()
        {
            try
            {
                // Make HTTP GET request to the Cars API (synchronous)
                var response = _httpClient.GetAsync(Constants.CarsApiUrl).Result;

                // Throw an exception if the HTTP status is not 2xx
                response.EnsureSuccessStatusCode();
                
                // Read the API response content as a string (synchronous)
