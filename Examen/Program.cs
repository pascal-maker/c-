using CTANDAI.CarReservationSystem.Services;   // Import the namespace that contains the CarService and CarReservationService
using CTANDAI.CarReservationSystem.Repositories; // Import repositories (data access layer)
...
namespace CTANDAI.CarReservationSystem
{
    class Program
    {
        // Entry point of the application (async because we will call async methods like ImportCarsFromApiAsync)
                ...
                try
                {
                    ...
                    switch (choice)
                    {
                        case "1":
                            ...
                            // This method is async, so we await it to complete before continuing
                            break;
        ...
        // Method to import cars from the API (async because it calls an API)
        static async Task ImportCarsFromApi(CarService carService)
        // This method fetches cars from an external API and saves them to the database
        {
            Console.WriteLine("Importing cars from API...");
            var cars = await carService.ImportCarsFromApiAsync(); // Fetch and save cars
